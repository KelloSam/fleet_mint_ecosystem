defmodule FleetMintWeb.TwoFactorController do
  ...
  alias FleetMint.Identity.TwoFactor
  ...
  def confirm(conn, %{"totp" => %{"code" => code}}) do
    ...
    with true <- not is_nil(user_id),
         ...
         true <- TwoFactor.valid_totp?(user, code) do
  ...
  def setup(conn, _params) do
    user = Users.get_user!(conn.assigns.current_user.id)
  ...
  def enable(conn, %{"totp" => %{"code" => code}}) do
    ...
    with true <- is_binary(encoded_secret),
         ...
         true <- TwoFactor.valid_totp_for_secret?(secret, code),
         {:ok, _} <- TwoFactor.enable_totp(user, secret) do
