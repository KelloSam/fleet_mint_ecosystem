defmodule FleetMintWeb.CashingReportController do
  ...
  def show(conn, %{"id" => id}) do
    cashing_report =
      Finance.get_cashing_report!(id) |> FleetMint.Repo.preload([:created_by, :updated_by])
  ...
  def unmatched(conn, _params) do
    cashing_reports =
      Finance.list_unreconciled_cashing_reports(organisation_id: conn.assigns.organisation_scope)
    ...
    with_organisation_access(conn, cashing_report.bus, ~p"/cashing_reports/unmatched", fn conn ->
      candidate_trips =
        if cashing_report.bus do
          Trips.list_trips_near_date(
            cashing_report.bus.organisation_id,
            cashing_report.report_date
          )
        else
          []
        end
  ...
  def match_trip(conn, %{"id" => id, "trip_id" => trip_id} = params) do
    ...
    with_organisation_access(conn, cashing_report.bus, ~p"/cashing_reports/unmatched", fn conn ->
      ...
      case Finance.match_cashing_report_to_trip(
             cashing_report,
             trip,
        ...
        {:error, :no_bus_on_report} ->
          ...
          |> put_flash(
            :error,
            "This report has no bus recorded, so it can't be matched to a trip."
          )
          |> redirect(to: ~p"/cashing_reports/unmatched")
