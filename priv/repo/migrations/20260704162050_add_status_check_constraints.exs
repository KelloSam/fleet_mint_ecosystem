defmodule FleetMint.Repo.Migrations.AddStatusCheckConstraints do
  ...
  def change do
    ...
    create constraint(:freight_clients, :freight_clients_client_type_check,
             check:
               "client_type IN ('mining_company','farm','general_business','individual','government','ngo')"
           )
    ...
    create constraint(:freight_orders, :freight_orders_status_check,
             check:
               "status IN ('pending','assigned','loading','in_transit','delivered','cancelled')"
           )

    create constraint(:freight_orders, :freight_orders_cargo_type_check,
             check:
               "cargo_type IN ('copper_ore','coal','cobalt_ore','agricultural_produce','maize','fertilizer','cement','fuel','general_cargo','hazardous','refrigerated','timber','steel')"
           )
    ...
    create constraint(:truck_profiles, :truck_profiles_truck_category_check,
             check:
               "truck_category IN ('rigid','articulated','tipper','flatbed','tanker','lowbed')"
           )

    create constraint(:vehicle_maintenances, :vehicle_maintenances_service_type_check,
             check:
               "service_type IN ('oil_change','tire_replacement','brake_service','full_service','engine_repair','transmission','gearbox','electrical','bodywork','inspection','other')"
           )
    ...
    create constraint(:operation_logs, :operation_logs_category_check,
             check:
               "category IN ('general','incident','maintenance','finance','staff','passenger')"
           )
